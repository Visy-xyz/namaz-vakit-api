...
Prayer-time JSON API for the Balkans, Western/Central Europe, the US and Canada
(36 countries) — including **Albania**, which the KMSH app depends on.

[![Production](https://img.shields.io/badge/API-namaz.frmsh.al-0366d6?style=flat-square)](https://namaz.frmsh.al/api/cities)

</div>

Sister repo: [**namaz-vakit-apiv2**](https://github.com/Visy-xyz/namaz-vakit-apiv2) serves
the rest of the world from `namazv2.frmsh.al`. The two country catalogs are disjoint, and
`lib/` and `api/` are kept **byte-identical** between them — fix a handler here, copy it there.
...
Base URL: `https://namaz.frmsh.al` (local: `http://localhost:3000` via `npm run dev`)

### `GET /api/cities`
Supported countries and cities.
- all: `/api/cities` (~64 KB gzipped)
- one country: `/api/cities?country=al` (**prefer this** — a few hundred bytes)
...
GET /api/prayer?country=al&city=tirana&date=2026-05-04
```
...
The fetch pipeline here is `scripts/fetch-europe.mjs` driven by `countries-all.json`
(the v2 repo uses `fetch-countries-allv2.mjs`); everything downstream is the same.
...
| Path | What |
|---|---|
| `api/` | Vercel serverless handlers — identical to the v2 repo (plus legacy `api/v1/` aliases) |
| `lib/` | Shared helpers — identical to the v2 repo |
